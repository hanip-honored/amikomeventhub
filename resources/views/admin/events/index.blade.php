    ...
    <div class="p-6">
        <div class="flex justify-between items-center mb-6">
            ...
            <a href="{{ route('admin.events.create') }}"
                class="bg-indigo-600 text-white px-4 py-2 rounded font semibold hover:bg-indigo-700">Tambah Event</a>
        </div>
        @if (session('success'))
                ...
                <tbody>
                    @foreach ($events as $event)
                        <tr class="border-b border-gray-100 hover:bg-gray 50">
                            ...
                            <td class="p-4 text-gray-600">{{ \Carbon\Carbon::parse($event->date)->format('d M Y, H:i') }}
                            </td>
                            <td class="p-4 flex gap-2">
                                <form action="{{ route('admin.events.destroy', $event->id) }}" method="POST"
                                    ...
                                    @method('DELETE')
                                    <button type="submit"
                                        class="bg-red-100 text-red-600 border border-red-200 px-3 py-1.5 rounded text-sm font semibold hover:bg-red-600 transition">Hapus</button>
                                </form>
                                <a href="{{ route('admin.events.edit', $event->id) }}"
                                    class="bg-blue-50 text-blue-600 border border-blue-200 px-3 py-1.5 rounded text-sm font-semibold hover:bg-blue-600 hover:text-white transition">Edit
                                    Data</a>
                            </td>
                        </tr>
                    @endforeach
